...
const Register: React.FC = () => {
  ...
  const callbacks = {
    // регистрация и перенаправление на страницу авторизации
    onSubmit: useCallback(async (user: FormValues) => {
      const response = await fetch(`https://front-test.hex.team/api/register?username=${user.register_name}&password=${user.register_password}`,
        {
          method: "POST",
          headers: {
      ...
      <div className="container">
        <form className="form" method="POST" onSubmit={handleSubmit(callbacks.onSubmit)} noValidate>
          ...
          <div className="form__wrapper">
            ...
            <input type="text" id="register_name" autoFocus placeholder="Margot Robbie" aria-label="введите ваше имя"
          ...
          <button className="form__button" type="submit" disabled={!isValid || isSubmitting}>Регистрация</button>
        </form>
      </div>
    </main>
  );
